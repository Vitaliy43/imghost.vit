...
<div class="item">
	<form class="ajaxForm uploadForm" action="/upload/fast" method="post" id="form{$i}">
		<input type="hidden" name="current_form" value="{$i}" id="current_form"/>
		...
		<div class="source-local-fields hidden" style="overflow: hidden;">
			...
			<fieldset style="border: 1px solid #000; padding: 5px;margin-bottom: 10px;">
				<legend style="margin-top: 5px;">{$language.PREVIEW}</legend>
				<label>
					<span style="margin-right: 15px;">{$language.PREVIEW_LEGEND}</span>
					<select name="TEXT_ON_PREVIEW" id="text_on_preview" onchange="show_text_preview(this);">
						<option value="none">{$language.NO_TEXT}</option>
						<option value="image_size">{$language.IMAGE_SIZE}</option>
						<option value="voluntary_text">{$language.VOLUNTARY_TEXT}</option>
					</select>
				</label>
				...
				<label style="margin-top:5px;">
					<span>{$language.PREVIEW_SIZE}</span>
					<input name="PREVIEW_SIZE" id="preview_size" value="{$preview_size}" type="text" size="3" onchange="validate_preview_size(this);"/>&nbsp;px
				</label>
			</fieldset>
			
			<input type="hidden" name="ACTION" value="upload_file" />
			<fieldset style="border: 1px solid #000; padding: 5px;margin-bottom: 10px;">
				<legend style="margin-top: 5px;">{$language.IMAGE}</legend>
			...
			<div class="more-options"><a class="doToggle" href="#" data-block=".more-options-block">{$language.ADDITIONAL_OPTIONS}</a></div>
			<fieldset class="more-options-block hidden">
				...
				<div class="field">
					<label for="jpeg_quality">{$language.JPEG_QUALITY}:</label>
					<div class="input">
						{$compression_jpeg_box} %
					</div>
				</div>
				<!--div class="field">
					<label for="png_quality">{$language.PNG_QUALITY}:</label>
					<div class="input">
						{$compression_png_box}
					</div>
				</div>
			...
			<div class="submit" style="display: none;">
				<input class="black-button" type="submit" value="{$language.EDIT}" style="font-size: 11px;" id="edit_file"/>
			</div>
		</div>
		{form_csrf()}
	</form>
