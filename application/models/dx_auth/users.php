...
class Users extends CI_Model {
	...
	function init($rolename){
		$query = $this->db->get_where('roles',array('name' => $rolename));
		if(count($query->row_array()) < 1)
			return;
		$this->role_id = $query->row()->id;
	}
...
// 		return $this->db->get($this->_table);
    ...
    function newpass($user_id, $pass, $key) {
        $data = array(
            'newpass' => $pass,
            'newpass_key' => $key,
            'newpass_time' => time() + $this->config->item('DX_forgot_password_expire')
        );
        return $this->set_user($user_id, $data);
    }
