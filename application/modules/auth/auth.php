...
//            $val->set_rules('email', lang("Email"), 'trim|required|min_length[3]|xss_clean|valid_email');
            $val->set_rules('login', lang("Username"), 'trim|required|min_length[5]|xss_clean');
            ...
            if ($val->run() AND $this->dx_auth->login($val->set_value('login'), $val->set_value('password'))) {
...
//                 ($hook = get_hook('auth_login_failed')) ? eval($hook) : NULL;
				...
				$data_login['info_message'] = $this->dx_auth->get_auth_error();
...
//                         ($hook = get_hook('auth_login_attemps_exceeded')) ? eval($hook) : NULL;
						...
						$data_login['cap_image'] = $this->dx_auth->get_captcha_image();
...
//                        $this->template->display('login_popup');
						  $response['content'] = $this->get_widget('login_popup',$data_login);
						  $response['answer'] = 0;
						  echo json_encode($response);
						  exit;
                    }
...
//             ($hook = get_hook('auth_user_is_logged')) ? eval($hook) : NULL;
	...
	function profile_edit(){
		...
		$res = $this->members->get_current_user();
		...
		'language' => $lang_auth,
		'user' => $user,
		'vk_domain' => $this->vk->main_domain
		);
		
		$options['0'] = $lang_main['ALL'];
		$buffer_options = array();
		$buffer_options['guest'] = $lang_auth['ACCESS_ROLE']['guest'];
		$buffer_options['user'] = $lang_auth['ACCESS_ROLE']['user'];
		$buffer_options['admin'] = $lang_auth['ACCESS_ROLE']['admin'];
		$options = form_dropdown('ACCESS_ROLE',$buffer_options,$user['access_role'],'class="combobox" style="width:200px;"');
		$data['access_roles'] = $options;

		$content = $this->get_widget('edit_profile',$data);
		$response['content'] = $content;
		echo json_encode($response);

	}
	
	public function register_with_seedoff(){
...
//			return false;
		if(isset($_POST['is_ajax'])){
			...
			$seedoff_info = $this->seedoff_sync->authorize($username,$password);
			if($seedoff_info){
				$response['answer'] = 1;
				$response['username'] = $seedoff_info['username'];
				$response['password'] = $password;
				$response['email'] = $seedoff_info['email'];
				$_SESSION['use_seedoff'] = 1;
				$_SESSION['seedoff_info']['role'] = $this->seedoff_sync->levels[$seedoff_info['id_level']];
				$_SESSION['seedoff_info']['avatar'] = $seedoff_info['avatar'];
			}
			else{
				$response['answer'] = 0;
				$response['content'] = $language['AUTH_SEEDOFF_FAILED'];
			}
		}
		$data = array(
		'language' => $language
		);
		$response['content'] = $this->get_widget('register_with_seedoff',$data);
		echo json_encode($response);
		}
		else{
    ...
    public function register() {
        ...
        $this->template->registerMeta("ROBOTS", "NOINDEX, NOFOLLOW");
		$language = Language::get_languages('auth');
		$this->site_title = $language['REGISTER'];
...
//                 ($hook = get_hook('auth_show_success_message')) ? eval($hook) : NULL;
                ...
                $this->template->assign('info_message', $this->dx_auth->get_auth_error());
				$data_register['info_message'] = $this->dx_auth->get_auth_error();
				if(isset($_SESSION['use_seedoff'])){
					$data_register['use_seedoff'] = 1;
					unset($_SESSION['use_seedoff']);
				}
				else
					$data_register['use_seedoff'] = 0;
				
				$data_register['language'] = $language;

                // Is registration using captcha
                if ($this->dx_auth->captcha_registration) {
                    $this->dx_auth->captcha();
                    $this->template->assign('cap_image', $this->dx_auth->get_captcha_image());
					$data_register['cap_image'] = $this->dx_auth->get_captcha_image();
                }
...
//                    $this->template->show('register');
					if(isset($_REQUEST['with_seedoff']))
						$data_register['with_seedoff'] = 1;
					else
						$data_register['with_seedoff'] = 0;

					  $this->template->assign('content',$this->get_page('register',$data_register));
					  $this->display_layout();
                } else {
...
//         ($hook = get_hook('auth_on_forgot_pass')) ? eval($hook) : NULL;
        ...
        if ($val->run() AND $this->dx_auth->forgot_password($val->set_value('email'))) {
			...
			$data_forgot['info_message'] = $data['auth_message'];
        }

        if ($this->dx_auth->_auth_error != NULL) {
            $this->template->assign('info_message', $this->dx_auth->_auth_error);
			$data_forgot['info_message'] = $this->dx_auth->_auth_error;
        }
...
//            $this->template->show('forgot_password');
			  $content = $this->get_page('forgot_password',$data_forgot);
//			  echo $content;exit;
...
//             ($hook = get_hook('auth_reset_pass_restored')) ? eval($hook) : NULL;
            ...
            $this->template->assign('content', $this->get_page('reset_password',array('auth_message' => $data['auth_message'])));
			$this->display_layout();
//            $this->template->show();
        } else {
