 ...
 function compare_date($data1, $data2, $parameter){
  ...
  if(!function_exists('cut_zero')) :
   ...
   if(!function_exists('time_text_data')) :
   	...
   	function time_text_data($date_from_db, $reverse = false){
   	...
   	function time_convert_date_to_text($date_from_db){
  ...
  function data_to_db($data,$add_time=false)
	...
	function parse_fb_time($data){
		$buffer = explode('T',$data);
		$buffer_time = explode('+',$buffer[1]);
		$time = $buffer_time[0];
		$new_data = $buffer[0].$time;
		return strtotime($new_data);
	}
