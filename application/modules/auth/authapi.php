...
//            $this->form_validation->set_rules('login', lang('Username', 'auth'), 'trim|required|min_length[3]|xss_clean');
            ...
            $doLoginResult = $this->dx_auth->login($this->input->post('email'), $this->input->post('password'));
            ...
            $jsonResponse['redirect'] = false;
            $jsonResponse['status'] = false;
            $jsonResponse['msg'] = $lang_auth['User is already logged in'];
        }
        ...
        if (!$this->dx_auth->is_logged_in() AND $this->dx_auth->allow_registration) {
            ...
            if ($val->run($this) AND $last_user = $this->dx_auth->register($val->set_value('username'), $val->set_value('password'), $val->set_value('email'), '', $key, '')) {
				...
				if(isset($_SESSION['use_seedoff'])){
					$res_update = $this->members->update_from_seedoff($last_user['id_user'],$_SESSION['seedoff_info']);
					/*
					$data = array(
					'id' => null,
					'name' => 'seedoff_net',
					...
					'access' => 'public',
					'user_id' => $last_user['id_user'],
					'password' => ''
					);
					if($res_update)
						$this->db->insert($this->my_albums->tablename,$data);
						*/
				}
				...
				if(class_exists('SUserProfileQuery')){
...
//                $json['additional_info']['email_activation'] = $this->dx_auth->email_activation;
				...
				$json['msg'] = str_replace('valid_email',$language['ERROR_VALID_EMAIL'],$json['msg']);
				$json['msg'] = str_replace('min_length',$language['ERROR_MIN_LENGTH'],$json['msg']);
	...
	public function edit_profile(){
            ...
            // Set form validation rules
            $val->set_rules('email', lang("Email"), 'trim|xss_clean|valid_email');
            $val->set_rules('show_name', $lang_auth['SHOW_NAME'], 'trim|xss_clean');
            $val->set_rules('birthday_year', '', 'trim|xss_clean');
            $val->set_rules('birthday_month', '', 'trim|xss_clean');
            $val->set_rules('birthday_day', '', 'trim|xss_clean');	
			$available_email = $this->email_check($this->input->post('email'));
			$valid_email = filter_var($this->input->post('email'),FILTER_VALIDATE_EMAIL);
			
            if ($val->run($this) AND $this->my_auth->edit_profile($res,$available_email) AND $valid_email) {
                // Set success message accordingly
                ...
                $json['status'] = true;
				$json['success'] = 1;
                $json['msg'] = $lang_auth['EDIT_PROFILE_SUCCESS'];
//                $json['refresh'] = $this->input->post('refresh') ? $this->input->post('refresh') : false;
//                $json['redirect'] = $this->input->post('redirect') ? $this->input->post('redirect') : false;
				$json['is_profile'] = 1;
				$new_user = $this->members->get_current_user();
				$json['email'] = $new_user->row()->email;
				$json['show_name'] = $new_user->row()->show_name;
...
//                $json['additional_info']['email_activation'] = $this->dx_auth->email_activation;
                ...
                $json['msg'] = validation_errors();
				if(!$valid_email)
					$json['msg'] = $lang_auth['ERROR_VALID_EMAIL'];
                $json['validations'] = array(
                    'email' => form_error('email'),
                    'show_name' => form_error('show_name')
                );
                $json['status'] = false;
                $json['anotherone'] = false;
                echo json_encode($json);
