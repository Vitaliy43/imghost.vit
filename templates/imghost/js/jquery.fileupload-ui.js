...
var msg_maximum_numbers = 'Превышено допустимое кол-во загружаемых изображений';
var too_big = 'Загружаемый файл слишком велик!';
var too_small = 'Загружаемый файл слишком мал!';
var filetype_wrong = 'Тип файла недопустим';
var counter = 1;
...
(function (factory) {
    ...
    $.widget('blueimp.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            add: function (e, data) {
				$('#faq_upload').show();
				var option = $('#list_templates option:selected').val();
				var word = $('#list_templates option:selected').text();
                ...
                $(this).fileupload('process', data).done(function () {
					...
					set_position();
					if($('.tr:last').find('.overlay').length > 0){
					
					}
					else{
						if(option != 0 && $('#batch').prop('checked') == true){
							createOverlay(word,1);
            ...
            // Callback for successful uploads:
				...
				$("#upload-drop-zone .error").each(function(){
					var text = $(this).text();
					if($(this).parent().prev().find('.error').length > 0){
						
						
					}
					else{
						if(text.indexOf('Empty file') != -1)
							$(this).parent().remove();
					}
...
//					set_torrents_list();
		...
		_onDrop: function (e) {
			$('#upload-drop-zone').css('background','inherit');
            var that = this,
        ...
        _onDragOver: function (e) {
			$('#upload-drop-zone').css('background','#aba9aa');
//			$('#upload-drop-zone').css('background','url("http://imghost.vit/templates/images/356_02.png")');
        ...
        _cancelHandler: function (e) {
			...
			that._adjustMaxNumberOfFiles(max_number_of_files);
