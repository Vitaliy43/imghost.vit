...
function show_add_tag(object){
	$(object).next().show();
	$(object).next().find('input').focus();
}

function submit_add_tag(parent_id,object){
	
	var url = document.location.href + '/add';
	var tag_name = $(object).prev().val();
	var	ajax_url = '/templates/administrator/images/ajax-loader.gif';

	var container = $('#container_tree').html();
	if(tag_name == ''){
		alert('Поле названия тега пустое!');
		return false;
	}
	...
	$.ajax({
		type: "POST",
   	 	url: url,
		dataType: 'json',
		data: 'tag_name='+tag_name+'&parent_id='+parent_id,
    	cache: false,
		beforeSend:function(){
			$('#container_tree').html('<div class="ajax_loader" style="padding-left:40%;padding-top:35%;"><img src="'+ajax_url+'"></div>');	
		},
		error: function(){
			$('#container_tree').html(container);
    	...
    	success: function(data){
			if(data.error != ''){
				...
				$('#container_tree').html(data.content);
				...
				if(parent_id != 0){
					$('#tags_tree #folder_'+parent_id).parent().removeClass('expandable');
					$('#tags_tree #folder_'+parent_id).prev().removeClass('expandable-hitarea');
					$('#tags_tree #folder_'+parent_id).prev().addClass('collapsable-hitarea');
					$('#tags_tree #folder_'+parent_id).parent().addClass('collapsable');
					$('#tags_tree #folder_'+parent_id).next().show();
				}
...
function show_edit_tag(id,object){
	$(object).parent().next().show();
	$(object).parent().hide();
}

function reset_tags(object){
	$(object).parent().prev().show();
	$(object).parent().hide();
}

function delete_tag(id,parent_id,object){
	
	var url = document.location.href + '/delete';
	var	ajax_url = '/templates/administrator/images/ajax-loader.gif';
	var container = $('#container_tree').html();
	
	jConfirm('Вы уверены, что хотите удалить данный тег?','IMGHOST.PRO',function(r)
	
{
	...
	type: "POST",
    url: url,
	data: 'parent_id='+parent_id+'&tag_id='+id,
	dataType: 'json',
    cache: false,
    beforeSend: function() {
		$('#container_tree').html('<div class="ajax_loader" style="padding-left:40%;padding-top:35%;"><img src="'+ajax_url+'"></div>');	
    ...
    success: function(data){
		if(data.error != ''){
				$(object).parent().find('.name').html(container);
				alert(data.error);
			}
			else{
				$('#container_tree').html(data.content);
				...
				if(parent_id != 0){
					...
					$('#tags_tree #folder_'+parent_id).prev().addClass('collapsable-hitarea');
					$('#tags_tree #folder_'+parent_id).parent().addClass('collapsable');
					$('#tags_tree #folder_'+parent_id).next().next().next().show();
				}
...
function submit_edit_tag(id,parent_id,object){
	
	var url = document.location.href + '/edit';
	var tag_name = $(object).prev().val();
	var container = $(object).parent().find('.name').html();
	var	ajax_url = '/templates/administrator/images/ajax-loaders/small.gif';
	
	$.ajax({
		type: "POST",
   	 	url: url,
		dataType: 'json',
		data: 'tag_name='+tag_name+'&parent_id='+parent_id+'&tag_id='+id,
    	cache: false,
		beforeSend:function(){
			$(object).parent().find('.name').html('<img src="'+ajax_url+'">');
		},
		error: function(){
			$(object).parent().find('.name').html(container);
			alert('Ошибка загрузки!');
...
function show_image(object){
	...
	$.colorbox({href: object.href, open: true, width:"50%"});

}

function edit_image(object){
    	...
    	success: function(data){
			...
			$.colorbox({html: data.content, open: true, title: 'Редактирование профиля', width: '50%'});

		}
    	});
	}
	
function filter_position(object){
	
	var	ajax_url = '/templates/imghost/images/ajax-loaders/big_sync.gif';
	var container = $('#blocks_table').html();
	...
	$.ajax({
    	...
    	cache: false,
		beforeSend:function(){
			$('#blocks_table').html('<div class="ajax_loader" style="width:100%;text-align:center;"><img src="'+ajax_url+'"></div>');	
		},
		error: function(){
			alert('Ошибка загрузки!');
			$('#blocks_table').html(container);	

		},
    	success: function(data){
				$('#blocks_table').html(data);	
...
function set_position(object){
	var	ajax_url = '/templates/imghost/images/ajax-loaders/small.gif';
	var id = $(object).closest('.block').attr('data-id');
	var position = $('#pos_'+id+' option:selected').val();
	var container = $(object).parent().html();
	
	$.ajax({
		type: "POST",
   	 	url: '/admin/components/init_window/advertisement/set_position',
		dataType: 'json',
    	cache: false,
		beforeSend:function(){
			$(object).replaceWith('<div class="ajax_loader"><img src="'+ajax_url+'"></div>');	
    	...
    	success: function(data){
			if(data.access == 1){
				$(object).replaceWith(data.content);
			}
			else{
...
function delete_image_admin(object,id,message){
